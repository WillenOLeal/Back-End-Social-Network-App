...
export class UserResolver {
  ...
  async addFriend(
    ...
    const senderPlusReceiverId: SenderPlusReceiverId = {
      ...sender,
      receiverId: receiver.id,
    };

    const pubSubPromise = pubSub.publish(friendReq, senderPlusReceiverId);
    const friendsPromise = Friendship.insert({
  ...
  async confirmFriendRequest(
    ...
    if (friendship) {
      const result = await Friendship.update(
        { senderId: userId, receiverId: payload.userId, status: 0 },
        { status: 1 }
      );
